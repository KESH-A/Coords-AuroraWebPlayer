...
function MainLayout() {
  ...
  }, [isPlayerModalOpen, isSettingsOpen, isFavoritesOpen]);
  ...
  useEffect(() => {
    const anyOpen = isPlayerModalOpen || isSettingsOpen || isFavoritesOpen;
  ...
  return (
    <div className="h-screen w-full transition-colors duration-500 relative overflow-x-hidden overflow-y-visible" style={{ '--bg-base': bgColor || '#0f172a', color: textColor || '#f8fafc', backgroundColor: bgColor || '#0f172a' }}>
      ...
      <header className="fixed top-0 left-0 right-0 p-4 z-30 pointer-events-none">
        <div className={'w-full max-w-xl mx-auto flex items-center justify-between gap-2 p-2 rounded-full border border-white/20 shadow-xl pointer-events-auto backdrop-blur-md bg-slate-950/60 ' + surf}>
          ...
          <button 
            ...
            className={`w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center shrink-0 border border-white/10 active:scale-95 transition-all ${isFavoritesOpen ? 'ring-2 ring-pink-500/60' : ''}`} 
            title="Favorites"
          >
            <Heart className="w-5 h-5" style={{ color: isFavoritesOpen ? '#ec4899' : activeColor, fill: isFavoritesOpen ? '#ec4899' : 'transparent' }} />
          </button>
      ...
      <main ref={mainRef} onScroll={handleScroll} className="w-full h-full overflow-y-auto no-scrollbar pt-24 pb-32 px-4 max-w-xl mx-auto relative z-10">
        <div className="w-full animate-morph-translate">
          {/* Favoriler açıkken arkadaki liste boş kalmasın diye localTracks fallback eklendi */}
          <TrackList 
            tracks={filtered.length > 0 ? filtered : localTracks} 
            isLoading={isLoading} 
            scrollTop={scrollTop} 
            containerHeight={containerHeight} 
            onReorder={(ordered) => { if (filtered.length === localTracks.length) reorderTracks(ordered); }} 
          />
        </div>
      </main>
