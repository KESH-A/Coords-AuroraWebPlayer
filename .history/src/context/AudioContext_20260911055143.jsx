...
export const AudioProvider = ({ children }) => {
  ...
  // WEB AUDIO KESİNTİSİZ UYANDIRMA MOTORU
  const initWebAudio = () => {
    if (!audioCtxRef.current) {
      try {
        ...
        analyser.fftSize = 64;
        analyser.smoothingTimeConstant = 0.45; // Hassas ve hızlı tepki için 0.45
        
        const masterGain = ctx.createGain();

        slotA.current.crossOrigin = "anonymous";
        slotB.current.crossOrigin = "anonymous";
        ...
        // Baglanti Zinciri: Source -> CrossfadeGain -> EQ Filters -> MasterGain -> Analyser -> Destination
        srcA.connect(cgA);
        srcB.connect(cgB);
    ...
    // AudioContext Askıdaysa Anında Resume Et
  ...
  // Garanti Play ve Auto-Resume Tetikleyicisi
  const playTrack = (track, isBackOperation = false) => {
    ...
    if (currentTrackRef.current?.id !== track.id || audio.src !== src) {
    ...
    audio.play().then(() => {
      setIsPlaying(true);
      // Play tıklandığı an Web Audio pipeline'ı uyandırılır
      if (audioCtxRef.current) {
