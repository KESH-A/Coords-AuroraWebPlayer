...
export const DynamicCanvasBackground = () => {
  ...
  const initEntities = (width, height, mode) => {
    ...
    if (mode === 'stars') {
      ...
      for (let i = 0; i < 8; i++) {
        list.push(createShootingStar(width, height));
      }
    } else if (mode === 'explosions') {
      for (let i = 0; i < 3; i++) {
        list.push(...createExplosion(Math.random() * width, Math.random() * height, rawColor));
      }
    }
    ...
    const draw = () => {
      ...
      if (bgEffectMode === 'stars') {
        ...
        for (let i = 0; i < count; i++) {
          ...
          ctx.fillStyle = hexToRgba(rawColor, 0.8);
          ctx.beginPath();
      ...
      // 2. MOD: KAYAN YILDIZLAR (SHOOTING STARS)
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          if (s.x > w * 1.3 || s.y > h * 1.3) {
            entitiesRef.current[i] = createShootingStar(w, h);
          }
          ...
          lineGrad.addColorStop(0, hexToRgba(rawColor, s.opacity));
      ...
      // 3. MOD: PATLAMALAR (EXPLOSIONS)
        ...
        if (list.length < 20 && Math.random() < 0.05) {
          entitiesRef.current.push(...createExplosion(Math.random() * w, Math.random() * h, rawColor));
        }
      } 
      // 4. MOD: AKIŞKAN MERMER DALGALARI (LIQUID MARBLE)
        ...
        for (let i = 0; i < lines; i++) {
          ...
          ctx.strokeStyle = hexToRgba(rawColor, 0.15 + (i / lines) * 0.2);
