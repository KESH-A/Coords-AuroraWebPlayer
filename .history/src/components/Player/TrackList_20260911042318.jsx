...
export const TrackList = ({ tracks, scrollTop = 0, containerHeight = 600, isLoading = false, onReorder }) => {
  ...
  // RENK VE GRADIENT DINAMIK SISTEMI
  const isGradient = themeStyle === 'gradient' && Array.isArray(gradientColors) && gradientColors.length >= 2;
  
  const activeColor = isGradient ? gradientColors[0] : (accentColor || '#9333ea');
  
  // Oynatılan kartın arka planı için gerçek CSS gradient stringi
  const activeGradientStyle = isGradient
      ...
      {activeTrack && (
        ...
        >
          <div
            ...
            style={{
              background: activePlaying ? activeGradientStyle : 'rgba(255,255,255,0.04)',
              ...
              transition: 'transform 0.5s cubic-bezier(0.34, 1.25, 0.64, 1), opacity 0.4s ease, background 0.4s ease, box-shadow 0.4s ease, border-color 0.4s ease',
          ...
          >
            {activePlaying && (
              <div
                ...
                style={{
                  background: `linear-gradient(120deg, rgba(255,255,255,0.25), transparent 45%, rgba(255,255,255,0.15) 100%)`,
            ...
            >
              <div className="flex items-center gap-3.5 truncate">
                ...
                <div className="truncate flex-1 min-w-0">
                  ...
                  {isSelected && (
                    <div className="mt-1.5 h-[3px] bg-white/20 rounded-full overflow-hidden" style={{ willChange: 'width' }}>
                      <div
                        key={track.id}
                        className="h-full rounded-full"
                        style={{
                          width: progressPct + '%',
                          background: activeGradientStyle,
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <div
                  ...
                  style={{
                    background: isTrackPlaying ? 'rgba(255, 255, 255, 0.2)' : activeGradientStyle,
                    borderColor: isTrackPlaying ? 'rgba(255, 255, 255, 0.3)' : 'transparent',
                    boxShadow: isTrackPlaying ? `0 0 12px ${activeColor}80` : `0 2px 10px ${activeColor}40`,
                  }}
                >
                  {isTrackPlaying ? (
