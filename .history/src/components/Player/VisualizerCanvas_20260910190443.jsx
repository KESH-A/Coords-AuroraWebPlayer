...
export const VisualizerCanvas = () => {
  ...
  // Sadece ilk 7 aktif frekans kanalını alıyoruz
  const activeBarsCount = 7;
  const totalBars = activeBarsCount * 2; // Aynalama ile toplam 14 sütun
  const slotWidth = w / totalBars;
  const barWidth = Math.max(2, slotWidth * 0.7);
  const gap = slotWidth - barWidth;
  const radius = Math.min(barWidth / 2, 4);

  // Sol taraf için ters, sağ taraf için düz aynalama
  for (let i = 0; i < totalBars; i++) {
    const freqIndex = i < activeBarsCount 
      ? (activeBarsCount - 1 - i) // Sol taraf
      : (i - activeBarsCount);     // Sağ taraf (Ayna)
  ...
  for (let i = 0; i < totalBars; i++) {
    ...
    const drawFluid = (w, h) => {
      ctx.clearRect(0, 0, w, h);
      const particleCount = Math.min(Math.floor(bufferLength / 4), 60);
      ...
      for (let i = 0; i < particleCount; i++) {
        // drawFluid içindeki val satırını değiştirin:
    ...
    const drawVHS = (w, h) => {
      ...
      const barCount = Math.min(Math.floor(bufferLength / 2), 32);
