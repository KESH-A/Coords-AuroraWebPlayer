...
export const FavoritesPage = ({ tracks, onClose }) => {
  ...
  // Glass background: Koyu tonu düşürüp blur oranını artırdık
  ...
  return (
    <div className={`fixed inset-0 z-[250] overflow-hidden rounded-none flex flex-col animate-fade-in ${surfaceClass}`}>
      <div className="flex items-center gap-3 p-4 pt-6">
        <button
          onClick={onClose}
          className="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center active:scale-95 transition-all border border-white/15"
        >
        ...
        <div className="ml-auto flex items-center gap-2">
          {favTracks.length > 0 && (
            <>
              <button onClick={() => setIsSelectMode(!isSelectMode)} className="text-[10px] font-bold px-2.5 py-1 rounded-full transition-all border border-white/10" style={{ backgroundColor: isSelectMode ? activeColor : 'rgba(255,255,255,0.1)', color: isSelectMode ? '#000' : '#94a3b8' }}>
                {isSelectMode ? 'Cancel' : 'Select'}
              </button>
              {isSelectMode ? (
                <button onClick={() => { selectedIds.forEach(id => toggleFavorite(id)); setSelectedIds([]); }} className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-red-500/80 text-white">Remove ({selectedIds.length})</button>
              ) : (
                <button onClick={() => { if (window.confirm('Clear all favorites?')) clearAllFavorites(); }} className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-red-500/80 text-white">Clear All</button>
              )}
            </>
          )}
            ...
            {favTracks.map((track, index) => {
              ...
              return (
                <div
                  ...
                  className="flex items-center justify-between p-3 rounded-2xl cursor-grab active:cursor-grabbing border border-white/15 bg-white/10 hover:bg-white/15 transition-all active:scale-[0.98] gpu-layer backdrop-blur-md shadow-lg"
                >
                  {isSelectMode && (
                  ...
                  <div className="flex items-center gap-3 truncate">
                    <div className="w-11 h-11 rounded-xl overflow-hidden bg-slate-900 border border-white/20 shrink-0 shadow-md">
                      <img
                    ...
                    <div className="truncate">
                      ...
                      <p className="text-xs text-slate-300 truncate">{track.artist || 'Local Track'}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <div
                      ...
                      className="w-9 h-9 rounded-full flex items-center justify-center border transition-all cursor-pointer shadow-md"
                      style={{ backgroundColor: playing ? activeColor : 'rgba(255,255,255,0.15)', borderColor: playing ? activeColor : 'rgba(255,255,255,0.2)' }}
                    >
