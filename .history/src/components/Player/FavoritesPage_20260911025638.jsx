...
export const FavoritesPage = ({ tracks = [], onClose }) => {
  ...
  return (
    <div 
      className={`fixed inset-0 z-[250] overflow-hidden rounded-none flex flex-col animate-fade-in select-none ${
        isGlass 
          ? 'bg-[#0a0f1d]/70 backdrop-blur-2xl border border-white/10' 
          : 'bg-[#070913]/95 backdrop-blur-xl'
      }`}
      style={{ color: textColor || '#ffffff' }}
    >
      {/* Header */}
      <div className="flex items-center justify-between p-4 pt-6 max-w-lg mx-auto w-full z-10">
        <div className="flex items-center gap-3">
          <button
            onClick={onClose}
            className="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center active:scale-95 transition-all backdrop-blur-md"
          ...
          {favTracks.length > 0 && (
            <>
              <button 
                ...
                className="text-[11px] font-bold px-3 py-1 rounded-full transition-all backdrop-blur-md" 
                style={{ 
                  backgroundColor: isSelectMode ? activeColor : 'rgba(255,255,255,0.1)', 
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-4 pb-28 max-w-lg mx-auto w-full relative">
        {favTracks.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-3 relative z-10">
            <div className="p-4 rounded-full bg-white/5 border border-white/10 backdrop-blur-md shadow-2xl">
              <Heart className="w-12 h-12 opacity-40" style={{ color: activeColor }} />
            </div>
            <p className="text-sm font-semibold tracking-wide text-slate-300">No favorites yet</p>
            <p className="text-xs text-slate-500">Tap the heart on any track to add it here</p>
          </div>
        ) : (
            ...
            {favTracks.map((track, index) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center justify-between p-3.5 rounded-3xl cursor-grab active:cursor-grabbing border transition-all duration-300 active:scale-[0.98] ${
                    playing 
                      ? 'bg-purple-900/40 border-purple-500/50 shadow-[0_0_25px_rgba(168,85,247,0.35)]' 
                ...
                >
                  ...
                  <div className="flex items-center gap-3.5 truncate">
                    <div className="w-12 h-12 rounded-2xl overflow-hidden bg-slate-900/80 border border-white/15 shrink-0 shadow-lg">
                      <img
                  ...
                  <div className="flex items-center gap-2.5 shrink-0">
                    <button
                      ...
                      className="w-9 h-9 rounded-full flex items-center justify-center active:scale-90 transition-all hover:bg-white/5"
                    >
                    ...
                    <div
                      ...
                      className="w-10 h-10 rounded-full flex items-center justify-center border transition-all cursor-pointer shadow-lg active:scale-95"
                      style={{ 
                        backgroundColor: playing ? activeColor : 'rgba(255,255,255,0.1)', 
                        borderColor: playing ? activeColor : 'rgba(255,255,255,0.15)',
