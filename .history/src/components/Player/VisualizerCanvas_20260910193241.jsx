...
export const VisualizerCanvas = () => {
  ...
  useEffect(() => {
    ...
    // CIRCULAR BEAT (Dairesel Ritim Tasarımı)
    const drawCircular = (w, h) => {
      ...
      // Dışa Doğru Yayılan Işınsal Barlar
    ...
    // RETRO / VHS MODU (Segmentli Dikey Barlar ve Süzülen Pik Parçacıkları)
    const drawVHS = (w, h) => {
      ctx.clearRect(0, 0, w, h);

      // VHS Tarama Çizgisi
      const scanOffset = (phaseRef.current * h * 0.4) % h;
      ctx.fillStyle = 'rgba(0, 255, 200, 0.06)';
      ctx.fillRect(0, scanOffset, w, 2);

      const mirrored = getMirroredActiveData();
      const barCount = mirrored.length;
      const slotWidth = w / barCount;
      const barWidth = Math.max(4, slotWidth * 0.5);
      const gap = slotWidth - barWidth;
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        if (val > 50) {
          ...
          ctx.beginPath();
          if (ctx.roundRect) {
            ctx.roundRect(x, capY, barWidth, 2.5, 1);
          } else {
            ctx.rect(x, capY, barWidth, 2.5);
          }
          ctx.fill();
        }
      }
