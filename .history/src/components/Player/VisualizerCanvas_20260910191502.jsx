...
export const VisualizerCanvas = () => {
  ...
  useEffect(() => {
    ...
    const drawFluid = (w, h) => {
      ...
      const baseRadius = Math.max(10, Math.min(w, h) * 0.25);
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        const targetRadius = baseRadius + (val / 255) * baseRadius * 0.5;
        const idx = i % heightsRef.current.length;
        ...
        ctx.beginPath();
        ctx.arc(Math.cos(angle) * r, Math.sin(angle) * r, Math.max(2, 3 + (val / 255) * 4), 0, Math.PI * 2);
        ctx.fillStyle = getColor(i, particleCount);
        ctx.fill();
      }

      const pulse = Math.sin(phaseRef.current * 2) * 0.05 + 0.95;
      const gradRadius = Math.max(1, baseRadius * pulse);
      ctx.beginPath();
